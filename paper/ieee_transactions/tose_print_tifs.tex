...
\author{Kamran~Khan
\thanks{K. Khan is an Independent Researcher based in Navi Mumbai, Maharashtra 410206, India (e-mail: kamrankhan.sde@gmail.com).}%
...
\begin{abstract}
Large-scale automated fingerprint identification systems (AFIS) face a long-standing trilemma among indexing scalability, non-linear distortion robustness, and cancelable template privacy. Conventional minutiae-based representations yield variable-length point clouds that require computationally expensive pairwise graph alignment ($O(N)$ exhaustive scans), severely inhibiting sub-linear retrieval in national-scale identity databases. While modern deep-learning embeddings generate fixed-length representations, they remain opaque, computationally demanding on edge sensors, and fragile against heavy physical alterations. In this paper, we propose \textbf{TOSE-Print}, a mathematically grounded, fixed-length ($d = 261$) biometric representation that encodes continuous structure-tensor ridge flow and skeletal topological invariants on the special Euclidean group $SE(2)$. By combining spatially pooled doubled-angle orientation fields, Betti-0 connected components, singular degree distributions, and 1D contour cycle spectra, TOSE-Print produces a compact, unit $L_2$-normalized vector inherently invariant to non-linear photometric fluctuations and minor spatial misalignments. To guarantee template privacy, we integrate a cancelable \textbf{Random Projection BioHashing} scheme proven to preserve angular cosine metrics via the Johnson-Lindenstrauss lemma while satisfying ISO/IEC 24745 unlinkability ($D_{\leftrightarrow}^{sys} = 0.0966$) and instantaneous revocability. For ultra-fast retrieval, TOSE-Print vectors are indexed using \textbf{Hierarchical Navigable Small World (HNSW)} proximity graphs. Rigorous evaluation across $1,800$ altered queries from the real SOCOFing benchmark against an enrolled gallery of $2,000$ fingerprints demonstrates \textbf{100.00\% Rank-1 identification accuracy} under Easy alterations and \textbf{98.00\% Rank-1} under severe physical obliteration. Scalability benchmarks across $1,000$ to $100,000$ fingerprints empirically validate a \textbf{$5.0\times$ sub-linear speedup} over linear scans with over $2,300$ queries per second (QPS) at $97\%$\textendash$100\%$ Recall@1.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{ISO/IEC 24745 Compliant Cancelable BioHashing}: We replace cryptographically flawed hash schemes with a user-token-seeded orthonormal Random Projection BioHashing framework. We provide the mathematical proof that our scheme strictly preserves angular cosine distances (via the Johnson-Lindenstrauss lemma) while satisfying unlinkability ($D_{\leftrightarrow}^{sys} = 0.0966$, distribution overlap $= 85.76\%$) and immediate revocability.
    \item \textbf{Empirical Sub-Linear Scalability}: We couple fixed-length TOSE-Print vectors with Hierarchical Navigable Small World (HNSW) proximity graphs, demonstrating an empirical $5.0\times$ query acceleration and over $2,300$ QPS at $100\%$ Recall@1 across $50,000$ gallery vectors.
...
We represent the local ridge direction in doubled-angle Cartesian vector coordinates weighted by coherence:
\begin{align}
    u_c(x, y) &= \kappa(x, y) \cos(2\theta(x, y)) = \frac{J_{xx} - J_{yy}}{J_{xx} + J_{yy} + \epsilon} \, \kappa(x, y), \\
    u_s(x, y) &= \kappa(x, y) \sin(2\theta(x, y)) = \frac{2J_{xy}}{J_{xx} + J_{yy} + \epsilon} \, \kappa(x, y).
\end{align}
This vector mapping is completely continuous across the entire image plane, mathematically smooth on $SE(2)$, and linear under spatial convolution.
...
\end{equation}
The sub-vectors are subsequently combined using optimal non-negative variance weights $\mathbf{w} = [w_{\text{spatial}}, w_{\text{ori}}, w_{\text{loop}}, w_{\text{topo}}, w_{\text{patch}}]^T$:
\begin{equation}
...
\end{equation}
Because $\sum w_i = 1.0$, the resulting composite vector $\hat{\mathbf{v}}_{\text{TOSE}}$ satisfies $\|\hat{\mathbf{v}}_{\text{TOSE}}\|_2 = 1.0$. The cosine similarity between two templates directly equals their inner product:
\begin{equation}
...
To guarantee compliance with the ISO/IEC 24745 biometric privacy standard \cite{b39, b43}, the raw TOSE-Print vector $\hat{\mathbf{v}} \in \mathbb{R}^d$ ($d=261$) is transformed into a protected binary representation $\mathbf{b} \in \{0, 1\}^m$ ($m=512$). 

Let a user-specific secret token key $K \in \mathbb{Z}^+$ seed a cryptographically secure pseudo-random number generator (PRNG). We construct an orthonormal projection matrix $\mathbf{R} \in \mathbb{R}^{m \times d}$ whose rows constitute normalized random hyperplanes in $\mathbb{R}^d$:
\begin{equation}
    \mathbf{R}_i \sim \mathcal{N}(0, \mathbf{I}_d), \quad \mathbf{R}_i \leftarrow \frac{\mathbf{R}_i}{\|\mathbf{R}_i\|_2}, \quad \forall i \in \{1, \dots, m\}.
\end{equation}
The continuous TOSE-Print vector is projected onto the random hyperplanes and quantized with a threshold $\tau = 0$:
\begin{equation}
    b_i = \begin{cases} 1, & \text{if } \langle \mathbf{R}_i, \hat{\mathbf{v}} \rangle > \tau, \\ 0, & \text{otherwise.} \end{cases}
\end{equation}

\subsection{Proof of Metric Similarity Preservation}
\textbf{Theorem 1} (Angular Distance Preservation): \textit{Let $\hat{\mathbf{v}}_1, \hat{\mathbf{v}}_2 \in \mathbb{S}^{d-1}$ be two unit-normalized TOSE-Print vectors with angle $\theta = \arccos(\langle \hat{\mathbf{v}}_1, \hat{\mathbf{v}}_2 \rangle)$. The expectation of the normalized Hamming distance between their BioHash codes generated with the same projection matrix $\mathbf{R}$ is strictly proportional to $\theta$:}
\begin{equation}
...
\textit{Proof}: The probability that a random hyperplane with normal vector $\mathbf{r} \sim \mathcal{N}(0, \mathbf{I}_d)$ separates two vectors $\hat{\mathbf{v}}_1$ and $\hat{\mathbf{v}}_2$ depends entirely on whether $\mathbf{r}$ falls within the dihedral angle formed by the hyperplanes orthogonal to $\hat{\mathbf{v}}_1$ and $\hat{\mathbf{v}}_2$. By rotational symmetry of the Gaussian distribution in $\mathbb{R}^d$, the projection of $\mathbf{r}$ onto the 2D plane spanned by $\hat{\mathbf{v}}_1$ and $\hat{\mathbf{v}}_2$ is uniformly distributed over the unit circle $S^1$. The probability of $\operatorname{sign}(\langle \mathbf{r}, \hat{\mathbf{v}}_1 \rangle) \ne \operatorname{sign}(\langle \mathbf{r}, \hat{\mathbf{v}}_2 \rangle)$ is therefore exactly the fraction of the circle subtended by angle $\theta$:
\begin{equation}
    \Pr[b_{1, i} \ne b_{2, i}] = \frac{2\theta}{2\pi} = \frac{\theta}{\pi}.
\end{equation}
By linearity of expectation across all $m$ independent hyperplanes:
\begin{equation}
    \mathbb{E}\left[ \frac{1}{m} \sum_{i=1}^m \mathbb{I}(b_{1, i} \ne b_{2, i}) \right] = \frac{1}{m} \sum_{i=1}^m \Pr[b_{1, i} \ne b_{2, i}] = \frac{\theta}{\pi}. \quad \blacksquare
\end{equation}
Consequently, normalized Hamming similarity $\mathcal{S}_H(\mathbf{b}_1, \mathbf{b}_2) = 1 - \frac{d_H}{m} \approx 1 - \frac{\theta}{\pi}$ monotonically preserves the original cosine proximity without requiring inversion of the biometric template.

\subsection{Security Analysis under ISO/IEC 24745}
\begin{enumerate}
    \item \textbf{Revocability}: If token key $K$ is compromised, the template is revoked immediately. Generating a replacement key $K_{\text{new}}$ constructs a new, uncorrelated projection matrix $\mathbf{R}^{\prime}$. The cross-matching similarity between the revoked and new template from the same fingerprint yields:
    \begin{equation}
        \mathbb{E}[\mathcal{S}_H(\mathbf{b}_K, \mathbf{b}_{K_{\text{new}}})] = 0.50,
    \end{equation}
    which matches the non-mated impostor baseline.
    \item \textbf{Unlinkability}: Distinct databases employ independent seed keys $K_A \ne K_B$. As demonstrated empirically in Section~\ref{sec:experiments}, the distribution of mated templates across different keys completely overlaps the non-mated impostor distribution ($D_{\leftrightarrow}^{sys} = 0.0966$).
    \item \textbf{Irreversibility}: Reconstructing $\hat{\mathbf{v}} \in \mathbb{R}^d$ from $\mathbf{b} \in \{0, 1\}^m$ requires inverting a system of $m$ non-linear inequality half-space constraints $\mathbf{R} \hat{\mathbf{v}} \gtrless 0$. Because the sign function discards absolute projection magnitudes, the solution space is an open polyhedral cone with infinitely many valid pre-images, making deterministic feature inversion mathematically ill-posed.
\end{enumerate}

\section{Sub-Linear Proximity Graph Indexing}
\label{sec:indexing}

To achieve real-time search across millions of enrolled fingerprints, we index unit $L_2$-normalized TOSE-Print vectors using \textbf{Hierarchical Navigable Small World (HNSW)} graphs \cite{b44}. An HNSW graph is a multi-layer geometric structure where each layer $\ell \in \{0, \dots, \ell_{\text{max}}\}$ contains a proximity graph with skip-list properties. Layer $\ell_{\text{max}}$ contains long-range highway edges for rapid coarse routing, while layer $0$ contains dense, short-range connections for fine nearest-neighbor convergence.

For unit vectors on the hypersphere $\mathbb{S}^{d-1}$, the inner product metric $\langle \mathbf{u}, \mathbf{v} \rangle$ directly induces the angular distance metric:
\begin{equation}
    \mathcal{D}_{\text{ang}}(\mathbf{u}, \mathbf{v}) = 1 - \langle \mathbf{u}, \mathbf{v} \rangle = \frac{1}{2} \|\mathbf{u} - \mathbf{v}\|_2^2.
\end{equation}
During index construction, each vector is inserted with a maximum degree $M = 32$ and an exploration breadth $efConstruction = 128$. For a query $\mathbf{q}$, greedy routing descends hierarchically through layers with search budget $efSearch = 64$. The time complexity of querying an enrolled gallery of size $N$ scales logarithmically:
\begin{equation}
    T_{\text{search}}(N) = O(\log N \cdot d),
\end{equation}
reducing query latencies from hundreds of milliseconds (in linear minutiae scans) to sub-millisecond durations.
...
To establish comparative rigor, we evaluated TOSE-Print against three standard baseline representations under identical subject-disjoint protocols:
\begin{enumerate}
...
While exact linear scans exhibit linear latency growth ($O(N)$), HNSW maintains sub-millisecond retrieval speeds up to $50,000$ vectors ($0.434$ ms/query, $5.0\times$ speedup), maintaining an exceptional $97.0\%$\textendash$100.0\%$ Recall@1. This empirically confirms that TOSE-Print enables million-scale watch-list identification on standard server hardware without GPU infrastructure.

\subsection{Cancelable Template Privacy and Unlinkability}
We evaluated the Random Projection BioHashing template protection scheme using the quantitative unlinkability framework defined by ISO/IEC 24745 \cite{b39}. Figure~\ref{fig:unlinkability} plots the score probability densities for: (1) genuine matches using identical keys, (2) non-mated impostors, and (3) mated samples under different revoked keys.
...
\textbf{Protected BioHash (Ours)} & \textbf{0.00\%} & \textbf{0.00\%} & \textbf{1.0000} & \textbf{0.0966} \\
\hline
\end{tabular}
\end{table}
...
\caption{ISO/IEC 24745 Unlinkability and Revocability score distributions for protected BioHash templates. The distribution of mated prints under different keys (blue) matches the non-mated impostor distribution (red) with $85.76\%$ overlap and $D_{\leftrightarrow}^{sys} = 0.0966$.}
\label{fig:unlinkability}
\end{figure}

Table~\ref{tab:privacy} demonstrates that protected BioHash templates achieve $0.00\%$ EER when verified with valid user keys. When a template is revoked and re-issued with a new key, the cross-matching similarity between the two templates drops to a mean of $0.5023$, which is statistically indistinguishable from random impostor pairings ($0.5001$). The quantitative unlinkability metric yields $D_{\leftrightarrow}^{sys} = 0.0966$ (ideal $\to 0$) with an \textbf{$85.76\%$ distribution overlap}, rigorously validating that protected templates cannot be linked across disparate service providers.
...
In this paper, we introduced \textbf{TOSE-Print}, a mathematically grounded, fixed-length geometric-topological representation for scalable and privacy-preserving fingerprint recognition. By formulating continuous structure tensors on $SE(2)$ alongside skeleton graph Betti invariants, TOSE-Print produces a compact, 261-dimensional unit $L_2$-normalized vector that overcomes the indexing limitations of traditional minutiae point clouds. We integrated an ISO/IEC 24745 compliant Random Projection BioHashing framework that mathematically preserves angular cosine distances while providing provable unlinkability ($D_{\leftrightarrow}^{sys} = 0.0966$) and instantaneous template revocation. Experiments over $1,800$ altered queries from the real SOCOFing benchmark against $2,000$ enrolled gallery prints demonstrated $100.00\%$ Rank-1 accuracy under Easy alterations and $98.00\%$ under severe physical obliteration. Furthermore, empirical FAISS HNSW benchmarks across $100,000$ templates confirmed a $5.0\times$ sub-linear query speedup with over $2,300$ QPS. 
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{author_kamran.png}}]{Kamran Khan}
received his degree in computer science and software engineering. He is currently an independent researcher in biometrics, computer vision, and scalable cryptographic indexing systems based in Navi Mumbai, India. His research interests include topological representation learning, cancelable biometrics, approximate nearest-neighbor search algorithms, and robust pattern recognition under non-linear physical distortion.
\end{IEEEbiography}
